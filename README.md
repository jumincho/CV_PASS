...
> 이 저장소에는 저작권상의 이유로 일부 이미지 자원과 폰트가 빠져 있고, Cloud Vision
> API 키 등 비밀 값은 포함되어 있지 않습니다. Firebase `google-services.json` 은 빌드가
> 바로 되도록 **placeholder** 가 커밋되어 있으니, 직접 실행하려면 자신의 Firebase
> 프로젝트 파일과 API 키로 교체하세요.
